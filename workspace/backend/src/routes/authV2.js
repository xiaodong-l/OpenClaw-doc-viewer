...
import { UserService, UserRole } from '../models/User.js';
  ...
  fastify.post('/auth/login', async (request, reply) => {
    try {
      ...
      if (user.mfaEnabled) {
        ...
        // 验证 MFA
        const mfaService = (await import('../services/mfa.js')).default
        const mfaResult = mfaService.verifyMFALogin(user, mfaToken)
        
        if (!mfaResult.valid) {
          return reply.code(401).send({ error: 'MFA 验证码无效' });
        }
      }
